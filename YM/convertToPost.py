...
def YMEventTranslate(event):
    for key, value in baseInfo.StatusMapYM.items():
        if(event.find(key) != -1):
            return value, YMCodeToName(value)
    return (None, None)
    


def YMPost(container, path):
    if(os.path.isfile(path+"ContainerInformation\\"+container+".csv")):
        with open(path+"ContainerInformation\\"+container+".csv") as containerInfo:
            ...
            for row in reader:
                ...
                postJson["eventCode"], postJson["eventName"] = YMEventTranslate(row[0])
